...
class ReservationForm extends ComponentBase
{
    ...
    public function onSubmit()
    {
        ...
        $data = Input::all();
        // Assign user_id and name from RainLab.User if logged in
        if (class_exists('RainLab\\User\\Facades\\Auth')) {
            $user = \RainLab\User\Facades\Auth::getUser();
            if ($user) {
                $data['user_id'] = $user->id;
                // Compose name from first_name and last_name, fallback to username or email
                ...
                $data['street'] = $user->direccio;
                $data['phone'] = $user->telefon;
            } else {
